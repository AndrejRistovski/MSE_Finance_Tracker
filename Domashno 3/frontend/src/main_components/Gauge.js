...
export default function StyledRadialGauge() {
    const [gaugeValue, setGaugeValue] = useState(0);

    const updateGaugeValue = (newValue) => {
        if (newValue >= 0 && newValue <= 100) {
            setGaugeValue(newValue); // Update state
        } else {
            console.error("Gauge value must be between 1 and 100.");
        }
    };

    React.useEffect(() => {
        updateGaugeValue(66); // Set to a new value here, between 1 and 100!
    }, []);
